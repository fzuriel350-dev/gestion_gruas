...
<head>
    ...
    @php
        ...
        $logo = $empresa->imageUrl($empresa->logo) ?? asset('logo-gruas.jpeg');
        $bgImage = $empresa->imageUrl($empresa->imagen_fondo);
        $empNombre = $empresa->nombre ?? config('app.name', 'Sistema de Grúas');
        $slogan = $empresa->slogan ?? 'Sistema Integral de Gestión de Grúas y Asistencia Vial';
        $quienesSomos = $empresa->quienes_somos ?? 'Somos una empresa dedicada a brindar servicios de grúas y asistencia vial, comprometida con la seguridad y satisfacción de nuestros clientes.';
        $mision = $empresa->mision ?? 'Brindar servicios de grúas y asistencia vial con rapidez, seguridad y profesionalismo, superando las expectativas de nuestros clientes y garantizando su tranquilidad en cada servicio.';
        $vision = $empresa->vision ?? 'Ser la empresa líder en servicios de grúas y asistencia vial en la región, reconocida por nuestra excelencia operativa, innovación constante y compromiso con la seguridad vial.';
        $prioridadTexto = is_array($empresa->prioridad) ? implode("\n", $empresa->prioridad) : ($empresa->prioridad ?? 'Atender cada servicio en el momento justo, con profesionalismo, eficacia y calidad, dando cuidado extremo durante el traslado, guarda y custodia, para así lograr la satisfacción total de nuestros clientes.');
        $valores = is_array($empresa->valores) ? $empresa->valores : [];
        if (empty($valores)) {
            $valores = [
                ['titulo' => 'RESPONSABILIDAD', 'descripcion' => 'Trabajando de la mano con las diferentes autoridades de manera adecuada, para así lograr la entera satisfacción de los usuarios y de la misma empresa.'],
                ['titulo' => 'CONFIANZA', 'descripcion' => 'Dar la tranquilidad a la ciudadanía a través de la tecnología de nuestros equipos y la capacitación de nuestro personal.'],
                ['titulo' => 'RESPETO', 'descripcion' => 'En todo momento y bajo cualquier circunstancia nuestro personal de cualquier área debe dirigirse al ciudadano de una forma adecuada brindando la atención que se merece.'],
            ];
        }
        $tiposServicio = $empresa->tiposServicio()->where('publicar', true)->get() ?? collect();
        $accesosRapidos = $empresa->accesosRapidos()->where('activo', true)->get() ?? collect();
    ...
    @if($logo)
    <link rel="prefetch" href="{{ $logo }}" as="image">
    @endif
...
<body class="antialiased" style="font-family: '{{ $font }}', sans-serif; background: linear-gradient(135deg, #000000, #1a1a1a, #0F0F0F);">
    @if($bgImage)
    <div class="fixed inset-0 bg-cover bg-center bg-no-repeat opacity-20" style="background-image: url('{{ $bgImage }}'); background-size: cover;"></div>
    @endif
    <div class="min-h-screen flex flex-col relative z-10">
        <header class="w-full px-6 py-4">
            <nav class="flex items-center justify-between max-w-7xl mx-auto">
                <div class="flex items-center gap-2">
                    <img src="{{ $logo }}" alt="Logo" class="h-14 md:h-20 w-auto object-contain" loading="lazy">
                </div>
        ...
        <section id="inicio" class="page-section flex-1 flex items-center justify-center px-6 py-12">
            <div class="text-center max-w-2xl">
                <h1 class="text-4xl lg:text-5xl font-extrabold text-white mb-3 animate-fade-in-up break-words">{{ $empNombre }}</h1>
                <p class="text-lg text-gray-400 mb-10 animate-fade-in-up delay-200">{{ $slogan }}</p>
        ...
        <section id="nosotros" class="page-section w-full py-20 px-6" style="background: linear-gradient(135deg, #0a0a0a, #1a1a1a);">
            <div class="max-w-5xl mx-auto">
                <div class="text-center mb-16 reveal">
                    ...
                    <p class="text-xl lg:text-2xl text-gray-300 leading-relaxed max-w-4xl mx-auto text-justify">{{ $quienesSomos }}</p>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
                    <div class="clay-card p-8 transition-all duration-300 reveal">
                        ...
                        <h3 class="text-xl font-bold mb-3" style="color:{{ $c }};">MISIÓN</h3>
                         <p class="text-gray-400 leading-relaxed text-justify">{{ $mision }}</p>
                    </div>
                    <div class="clay-card p-8 transition-all duration-300 reveal">
                        <div class="w-14 h-14 flex items-center justify-center rounded-xl mb-5" style="background:{{ $c }}33;">
                        ...
                        <h3 class="text-xl font-bold mb-3" style="color:{{ $c }};">VISIÓN</h3>
                         <p class="text-gray-400 leading-relaxed text-justify">{{ $vision }}</p>
                    </div>
                </div>
                <div class="max-w-4xl mx-auto mb-16 text-center reveal">
                    ...
                    <div class="clay-card p-8 transition-all duration-300 text-left">
                         <p class="text-gray-300 leading-relaxed text-lg text-justify">{{ $prioridadTexto }}</p>
                    </div>
                </div>
                <div class="text-center mb-10">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        @foreach($valores as $valor)
                            @php
                                $tituloValor = is_string($valor) ? strtoupper($valor) : (is_array($valor) ? ($valor['titulo'] ?? '') : '');
                                $descValor = is_array($valor) ? ($valor['descripcion'] ?? '') : '';
                            @endphp
                            @if($tituloValor)
                            <div class="clay-card p-6 text-left transition-all duration-300 reveal">
                                <h4 class="font-bold text-lg mb-2" style="color:{{ $c }};">{{ $tituloValor }}</h4>
                                @if($descValor)
                                <p class="text-gray-400 leading-relaxed text-justify">{{ $descValor }}</p>
                                @endif
                            </div>
                            @endif
                        @endforeach
                ...
                <div class="text-center reveal">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-3xl mx-auto">
                        @forelse($accesosRapidos as $acceso)
                         <a href="{{ $acceso->url }}" target="_blank" class="clay-card p-6 text-center transition-all duration-300 group">
                             <div class="w-14 h-14 mx-auto mb-3 flex items-center justify-center rounded-xl group-hover:scale-110 transition-transform duration-300" style="background:{{ $c }}33;">
                                 @if($acceso->icono_imagen)
                                 <img src="{{ $empresa->imageUrl($acceso->icono_imagen) }}" alt="{{ $acceso->titulo }}" class="w-8 h-8 object-contain">
                                 @elseif($acceso->icono)
                                 <i class="{{ $acceso->icono }}" style="color:{{ $c }};font-size:24px;"></i>
                                 @else
                                 <svg class="w-6 h-6" style="color:{{ $c }};" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>
                                 @endif
                             </div>
                             <h4 class="text-white font-bold">{{ $acceso->titulo }}</h4>
                         </a>
                        @empty
                    ...
                    @forelse($tiposServicio as $tipo)
                     <div class="clay-card p-8 text-center transition-all duration-300 hover:-translate-y-2 reveal">
                         <div class="w-16 h-16 mx-auto mb-5 flex items-center justify-center rounded-xl" style="background:{{ $c }}33;">
                             <svg class="w-8 h-8" style="color:{{ $c }};" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" /></svg>
                         </div>
                         <h3 class="text-xl font-bold text-white mb-3 break-words">{{ $tipo->nombre }}</h3>
                         @if($tipo->descripcion)
                         <p class="text-gray-400 leading-relaxed">{{ $tipo->descripcion }}</p>
                         @endif
                     </div>
                    @empty
                     <div class="col-span-full clay-card p-8 text-center reveal">
                         <p class="text-gray-400">No hay servicios configurados. Agrega tipos de servicio desde el panel de administración.</p>
                     </div>
                    @endforelse
                 </div>
            </div>
